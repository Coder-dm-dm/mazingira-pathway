...
# 3. Dynamic Dependency Syncing via requirements.txt using explicit .venv binaries
...
MODEL_CHECK=false
if [ -d "models" ]; then
    # Checks for any GGUF file in models/ or models/translator/
    if ls models/*.gguf 1> /dev/null 2>&1 || ls models/*/*.gguf 1> /dev/null 2>&1; then
        MODEL_CHECK=true
    fi
fi

if [ "$MODEL_CHECK" = false ]; then
...
# 6. Execute Application Launch using .venv Python
echo "=========================================================="
